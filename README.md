...
| p97 | #97 four equidistant vertices | a dihedral-symmetry exclusion theorem; exact Danzer coordinates; k=3 impossible for n <= 7 (n = 7 by solver verdicts, using veljjanoski's constraints); veljjanoski's k=4 certificates for n <= 9 checked |
...
| p98 | #98 distinct distances in general position | exact values D_gen(3..7) = 1, 2, 3, 4, 5 (the n = 6, 7 lower bounds re-derived on 2026-09-17 after the decider behind them proved unsound), the best known lower bound for 5 <= n <= 10 (tied at 4 and 11 to 13 with a later forum bound); D_gen(8) in [5,7] |
