...
- **Piepmeyer's example is itself an extremal few-distance set**: 9 points, exactly 4 distinct distances, and g(4) = 9. **[COMPUTED HERE]** (the 4-value count). **Update: it IS one of the Erdős–Fishburn extremal configurations**, the one they describe as "three equilateral triangles with the same center" and call a curiosity (their Theorem 1 and p. 116-117); e9.py shows the distance multisets agree exactly. Erdős–Fishburn do not mention Piepmeyer.
...
7. Whether Piepmeyer's 9-point set coincides with one of the Erdős–Fishburn extremal 4-distance configurations — **now checked: it does** (e9.py; NOTE.md section 3).
8. **Brass 1996 remains NOT OBTAINED** after a second attempt (OpenAlex: closed, no repository full text; no Wayback snapshot of the ScienceDirect page; no CORE record; Semantic Scholar lists it as bronze open access at the publisher only). Its abstract defines δ(n) and states an asymptotic result for sets in a half-strip; whether it lists any small exact values is unknown, and the comment says so.
